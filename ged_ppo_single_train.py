...
from src.ged_ppo_single_model import ActorNet, CriticNet, GraphEncoder
from utils.utils import print_args
from utils.tfboard_helper import TensorboardUtil
from ged_ppo_single_eval import evaluate
...
class ItemsContainer:
    ...
    def append(self, reward, inp_graph_1, inp_graph_2, ori_k, partial_cost, done, partial_x):
        ...
        self.__partial_cost.append(partial_cost)
        self.__done.append(done)
        self.__partial_x.append(partial_x)
    ...
    def partial_cost(self):
        return deepcopy(self.__partial_cost)
    ...
    @property
    def partial_x(self):
        return deepcopy(self.__partial_x)

    def update(self, idx, reward=None, inp_graph_1=None, inp_graph_2=None, partial_cost=None, done=None, partial_x=None):
        ...
        if partial_cost is not None:
            self.__partial_cost[idx] = partial_cost
        if done is not None:
            self.__done[idx] = done
        if partial_x is not None:
            self.__partial_x[idx] = partial_x
...
class ActorCritic(nn.Module):
    ...
    def act(self, inp_graph_1, inp_graph_2, partial_x, memory):
        node_feat_1, node_feat_2, graph_feat_1, graph_feat_2 = self.state_encoder(inp_graph_1, inp_graph_2, partial_x)
        actions, action_logits, entropy = self.actor_net(node_feat_1, node_feat_2, partial_x)

        memory.states.append((inp_graph_1, inp_graph_2, partial_x))
        memory.actions.append(actions)
        memory.logprobs.append(action_logits)

        return actions

    def evaluate(self, inp_graph_1, inp_graph_2, partial_x, action):
        node_feat_1, node_feat_2, graph_feat_1, graph_feat_2 = self.state_encoder(inp_graph_1, inp_graph_2, partial_x)
        _, action_logits, entropy = self.actor_net(node_feat_1, node_feat_2, partial_x, action)
...
class PPO:
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            logprobs, state_values, dist_entropy = self.policy.evaluate(old_graph_1, old_graph_2, old_partial_x, old_actions)
...
def main(args):
    ...
    for i_episode in range(1, args.max_episodes + 1):
        ...
        for b in range(args.batch_size):
            ...
            inp_graph_1, inp_graph_2, ori_k, _, baselines, _ = tuples_train[graph_index]
            partial_x = torch.zeros(inp_graph_1.num_nodes+1, inp_graph_2.num_nodes+1, device=device)
            partial_cost = 0
            items_batch.append(0, inp_graph_1, inp_graph_2, ori_k, partial_cost, False, partial_x)
        ...
        for t in range(args.max_timesteps):
            ...
            # Running policy_old:
            with torch.no_grad():
                action_batch = ppo.policy_old.act(
                    items_batch.inp_graph_1, items_batch.inp_graph_2, items_batch.partial_x, memory)

            def step_func_feeder(batch_size):
                partial_x = items_batch.partial_x
                batch_ori_k = items_batch.ori_k
                batch_partial_cost = items_batch.partial_cost
                for b in range(batch_size):
                    yield partial_x[b], batch_ori_k[b], action_batch[:, b], batch_partial_cost[b]

            if args.batch_size > 1:
                pool_map = mp_pool.starmap_async(ged_env.step_e2e, step_func_feeder(args.batch_size))
                step_list = pool_map.get()
            else:
                step_list = [ged_env.step_e2e(*x) for x in step_func_feeder(args.batch_size)]
            for b, item in enumerate(step_list):
                reward, partial_x, partial_cost, done = item  # only graph1 is updated
                items_batch.update(b, reward=reward, partial_x=partial_x, partial_cost=partial_cost, done=done)
        ...
        if i_episode % args.test_interval == 0:
            ...
            if test_dict["ratio"]["mean"] > best_test_ratio:
                ...
                file_name = f'./E2E_PPO_{args.solver_type}_dataset{args.dataset}' \
